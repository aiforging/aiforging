...
10. **`aiforging.github.io`** — a "Then you check it. OPTIONAL" subsection added to the Method section with two dashed cards and a gold-rule callout for the full-suite handoff; badge, JSON-LD `softwareVersion`, both FAQ copies, and the plugin note updated; install snippet extended with `forge` / `browser-testing` / `review-loop`; `sitemap.xml` lastmod bumped. **Note: the OneDrive copy at `OneDrive-CertainPath/ai-forging/website/index.html` was already stale before this session (Aug 13, 20 KB vs the repo's 30 KB) and was deliberately left alone.** Worth deciding whether that copy should be deleted or re-synced — two sources for one page is how the next edit gets lost.
...
12. **Audit pass, and what it caught.** An independent subagent audited the session's output against the repo before anything was committed. It found nine real defects, all fixed:
    ...
    - **The two new stages were documented as slash commands in ~15 places, but only existed as skills.** Skills are not `/`-invocable. Fixed by adding `commands/browser-testing.md` and `commands/review-loop.md` as thin pointers (the `forge.md` → `new-feature.md` pattern), which was also the shape the ask implied. **Related, still open:** `commands/setup.md` Step B.5 tells the user they can invoke `/aiforging:hammer-refactor` and `/aiforging:capture-pattern` — those are skills with no command wrapper, so that sentence is wrong. Left alone this session; either add two more pointers or fix the wording.
...
5. **Fix the `/aiforging:hammer-refactor` and `/aiforging:capture-pattern` wording in `commands/setup.md` Step B.5** — those are skills with no command wrapper, so the sentence telling users they can type them is wrong. Either add two more thin pointers (consistent with what Session 7 did for the verification stages) or reword. Caught in the Session 7 audit, deliberately left open.
6. Then reconsider Decision 20.
