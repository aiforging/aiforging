...
21. **Workspace-as-role: the forge workspace is not always a separate directory.** (Replaces the rigid "always a separate directory" assumption in Decision 13.) The forge workspace is a *role* — "the place where spec/plan files and the shared pattern library live" — that adapts to the user's repo topology:
    ...
    - **Detection**: `/aiforging:setup` begins with a scenario interview: "How is your codebase organized?" For multi-repo, it asks whether the user already has a repo where centralized planning docs live (use it) or wants to create one (Phase A). For monorepo/single, it operates in-repo. The workspace marker is the presence of `docs/features/README.md` with the AI Forging marker string — no new config file needed (same detection the current Phase A already uses, now applied to repos too). **Session 7 correction:** as implemented, the operative check is `./CLAUDE.md` containing `AI Forging workspace` plus a `docs/features/` directory — that is what `/aiforging:setup`, `/aiforging:new-feature`, and `/aiforging:update-targets` actually test, and the skills were aligned to it. A marker comment was also added to `templates/docs-features-README.md` so the originally-recorded marker exists too, but nothing depends on it. Prior wording in `hammer-refactor` referred to an 'AI Forging marker' in `docs/features/README.md` that had never been written.
...
24. **One named test suite per FEATURE — never per work item — and no agent ever runs the full repository suite.** The suite is registered before the first test and augmented by every subsequent work item, so one fast command always exercises the whole feature. Fire agents, Hammer agents, and review-loop fix agents all run that suite and only that suite. Documented in `conventions/tdd/feature-test-suite.md`.

    The rule is repeated in **four** places on purpose — the convention doc, `plan.md`'s `## Test suite` block, every slice's "Prompt for subagent" field, and the subagent-orchestration prompt templates. This looks redundant and is not: a fresh-context subagent reads the slice it was handed, and "run everything to prove I'm done" is the strongest default an agent has. It must be countermanded where the agent cannot miss it.

    **The regression risk is named and accepted, not hidden.** Scoping every run means a change made for feature A can break feature B invisibly. The mitigation is not a wider test run — it is (a) atomic per-slice commits so any regression is one `git revert` away, and (b) a **mandatory spoken handoff** at the end of implementation, in which the AI tells the human that every run was scoped, gives them the full-suite command, and stops. `hammer-refactor`, `browser-testing`, and `review-loop` all carry that message in their completion contract. The previous phrasing — a "recommendation" at the end of Hammer — was too weak: it is exactly the kind of note that gets skipped when a pass went well.

25. **The feature folder has six document roles, three conditional, plus two machine-written run-record directories.** `spec.md` and `plan.md` (always), `overview.md` (nested shape only), `testing.md` (whenever the feature has a UI surface), `summary.md` (optional, 3+ work items, written *after* implementation), `notes.md` (optional). Plus `ai-testing/NN/` and `ai-reviews/NN/` — numbered, append-only, never hand-edited.

    Ported from the project-hub-plus orchestrator, which had grown these organically. **`testing.md` is one per feature, never one per work item**, ordered by work item in dependency order for the nested shape — one checklist that walks the feature the way a user meets it beats five checklists nobody opens. It must be written *before* implementation ends: a checklist written afterwards describes what was built, one written from the spec describes what was supposed to be built, and the gap between them is the bug. Skipping it (research-only, internal pipeline change) is a decision **recorded in the spec**, because a missing `testing.md` with no explanation is indistinguishable from an oversight.

26. **Two optional verification stages, after the forge, in a fixed order: `browser-testing` then `review-loop`.** Both ported from project-hub-plus, both generalized for the forge workspace model (workspace resolution, targets derived from the scenario rather than hardcoded, no hardcoded base branches, browser-tooling-agnostic, review-capability-agnostic with a fallback rubric built from the target's own `.aiforging/` artifacts).

    - **`browser-testing` never fixes what it finds, and never invents a `testing.md`.** A failing step is evidence that the product and the spec disagree; it is not evidence about which is wrong, and that decision requires knowing what the feature is *for*. An auto-fixer would cement the wrong assumption *and* produce a green checklist attesting to it — worse than no browser testing at all. Findings go to `escalations.md` and to a conversation.
    - **`review-loop` triages before accepting.** Roughly a third of review findings describe deliberate behavior. It classifies by change class rather than felt confidence (a confident agent is exactly the one that does not ask), and stops on a real condition rather than a finding count or severity trend — both were tried in the reference implementation and both misled.
    - **Order matters.** Review rounds can run to near-exhaustion on a diff while a behavioral defect sits in the most-used screen. The cheapest moment to reverse a wrong decision is before ten work items are built on it.

    Both are **workspace-scoped**, not copied into targets: their inputs (`testing.md`, the feature folder, the target list) live at the workspace, and a copy in one target repo would be a skill whose primary input is not in that repo — discoverable and broken on invocation.

27. **`.claude-plugin/artifacts.json` is the single source of truth for everything the plugin copies out of itself.** `/aiforging:setup`, `/aiforging:update-targets`, and `/aiforging:uninstall` all read it instead of carrying their own lists.
...
Last updated: 2026-09-03 (Session 7 — v0.3.0 verification stages, scoped suites, artifact manifest)
...
- `templates/` — bootstrap templates for forge workspace init (Phase A):
  ...
  - `templates/feature-testing.md` — **new Session 7.** `testing.md` skeleton, copied per-feature by `/aiforging:new-feature`. Not installed into any scope.
- `conventions/` library (copied into each onboarded target as `.aiforging/`):
  ...
  - `conventions/tdd/` — `fire-red-green-refactor.md` (delegates to `superpowers:test-driven-development`), `feature-test-suite.md` (**new Session 7** — Decision 24), `test-harness-requirements.md`, `repository-testing.md`.
...
- `.claude-plugin/artifacts.json` — **new Session 7.** The artifact manifest (Decision 27). Every artifact the plugin copies out of itself: source, destination per scope, applicable target roles, optionality, install bundle, update policy, and `since` version. Plus a `user_owned` list nothing may touch. Read by `setup`, `update-targets`, and `uninstall`.
- `skills/browser-testing/SKILL.md` — **new Session 7.** Optional post-implementation stage (Decision 26). Walks a feature's `testing.md` in a real browser, marks 👤 items inline for the human to work in parallel, records one evidence line per step, writes `docs/features/<feature>/ai-testing/NN/`. Never fixes anything, never invents a checklist, local/named-QA environments only. Workspace-scoped.
- `skills/review-loop/SKILL.md` — **new Session 7.** Optional post-implementation stage (Decision 26). Rounds of review → triage → fix across implicated repos; parallel read-only reviews, serialized fixes, verified triage, class-based classification, convergence rules, real stop conditions, `escalations.md` surfaced every round. Writes `docs/features/<feature>/ai-reviews/NN/`. Workspace-scoped.
- `CONTRIBUTING.md` — **new Session 7.** Contribution guide: the field-report ask, what the project is opinionated about, the bar for a pattern file, stack-adapter requirements, the manifest rule, and the local dogfood loop.
- `.github/` — **new Session 7.** Issue templates (field report, pattern proposal), issue-template config, and a PR template whose checklist includes the manifest rule and the mirrored-docs rule.

### Designed (command file written, not yet dogfooded)

- `commands/update-targets.md` — `/aiforging:update-targets`. Propagates plugin-level updates into the workspace and all onboarded targets. Diff-and-ask semantics: nothing overwritten silently, per-item approval, timestamped backups. Handles Scenario A/B/C target discovery, seeded-vs-user-captured pattern distinction, pre-two-tier migration, and stale-path cleanup. Session 5. **Session 7: rewritten to be manifest-driven** (Decision 27) — Step 0.5 reads `artifacts.json`, additions are distinguished from updates by `since` and presented as explained offers, and Step 4 now reports what changed in *behavior* rather than only which files moved. Still not dogfooded end-to-end; that is the top item for Session 8.
...
- **Decision 20 (candidate) — upstream pattern propagation.** A target-local pattern that proves generally applicable should be promotable to the shared tier. Still unspecified. Deferred again in Session 7.
- **Pluggable specialist agents in the dispatch pipeline** (Srdjan's feedback #5, parked in Session 6). Still parked.
- **`/aiforging:capture-testing` or equivalent** — nothing currently helps a user *fill in* a `testing.md` from a completed spec; `/aiforging:new-feature` only scaffolds the skeleton, and the planning workflow's Step 4 asks a human-driven Claude to do it. Worth considering once `testing.md` has been used on a few real features.
...
### Session 7 — 2026-09-03 (v0.3.0 — verification stages, scoped test suites, artifact manifest)

Driven from the CertainPath project-hub-plus orchestrator, which had grown three things the open-source plugin didn't have: a hard test-scoping discipline, a richer feature-folder document set, and two post-implementation skills. This session ported all three and fixed the structural reason such improvements could strand existing users.

**Decisions 24-27 added.** Read those first — they carry the reasoning; the list below is what was actually changed.

1. **`conventions/tdd/feature-test-suite.md` — new.** One named suite per feature, registered before the first test, augmented by every work item. Per-stack registration syntax (PHPUnit, pytest, vitest/jest, Go, JUnit, .NET). The four places the rule is repeated, and why the repetition is load-bearing. An explicit statement of the regression risk and the mandatory spoken handoff that covers it.

2. **Scoping wired through everything that runs a test.** `conventions/tdd/fire-red-green-refactor.md` — its "Scoped test runs" section rewritten with the ⛔ block, the handoff, and two new refusals. `conventions/subagent-orchestration/README.md` — new rule 8, plus the scoped-run instruction added verbatim to all four prompt templates (generic / backend / frontend / hammer-refactor). `skills/hammer-refactor/SKILL.md` — Step 4 now passes the suite name and command into every dispatch, Step 5 skips re-verification when a slice changed no code, and Step 6 was rewritten from an optional "you may want to run the full suite" recommendation into a **mandatory, unconditional, spoken handoff.** That weakening was the actual defect: a recommendation at the end of a pass that went well is the note that gets skipped.

3. **Feature-folder convention extended with `testing.md` and `summary.md`** (`overview.md` already existed but was under-specified). Both shapes' layouts updated; a new "The files in a feature folder" section with a required/when/job table, the `testing.md` shape and the three sections it must cover in order, and the "one per feature, never one per work item" rule. `ai-testing/` and `ai-reviews/` documented as numbered append-only machine records. Lifecycle grew from 7 steps to 11, including both verification stages and the full-suite handoff. `templates/docs-features-README.md` mirrored throughout (per the CLAUDE.md mirroring rule). `plan.md` gained a `## Test suite` opening block and a `Test suite` reference in the slice template.

4. **`templates/feature-testing.md` — new.** The `testing.md` skeleton, with the 👤 legend, the three required sections, nested-shape guidance in a comment, and placeholder items. **`/aiforging:new-feature` Step 4c** asks whether the feature has a UI surface, copies the template if so, records the reason in the spec if not — and explicitly **does not fill in the checklist**, because the spec that produces those items doesn't exist yet at scaffolding time. Its extension flow now keeps `testing.md` at the feature level when converting flat → nested.

5. **`skills/browser-testing/SKILL.md` — new.** Ported from `project-hub-plus/.claude/skills/browser-testing/`. Generalizations for the plugin: workspace resolution via the three standard cases instead of a fixed repo layout; browser-tooling-agnostic (the source assumed a specific extension's `list_connected_browsers`); database access described in terms of the target's own documented connection rather than a known schema; the reference-implementation stories kept but anonymized, since they are the evidence for rules that look arbitrary without them. The source skill pointed at a design doc (`docs/porting/browser-testing-skill-design.md`) that end users will never have — that rationale is now inlined.

6. **`skills/review-loop/SKILL.md` — new.** Ported the same way. Biggest generalizations: **base-branch derivation** (the source hardcoded `qa` for two repos and a dated release branch for two others — now a per-repo derive-and-confirm step with the "never hardcode, neither heuristic is reliable alone" reasoning preserved), **repo discovery** from the workspace scenario, and **review-capability agnosticism** — the source invoked a specific `code-review` skill, so the plugin version keeps the verify-before-fanning-out rule but adds a fallback rubric built from artifacts AI Forging already installs (`spec.md`, the merged pattern tiers, `.aiforging/architecture/`, the usual defect classes). That fallback turned out to be the more on-brand design: review agents that cite a pattern file produce findings a fix agent can act on precisely.

7. **`.claude-plugin/artifacts.json` — new.** 16 entries. Fixes the reason `update-targets` kept drifting: three commands each carried their own hardcoded list, and an artifact missing from one of them would install on fresh setups and never reach existing users, silently. `update-targets` Steps 0.5/1/2 rewritten to walk the manifest; `setup` Step 0 and the new Step A.2.4 read it; `uninstall` now derives its removal list from it. Entries carry `since`, so an addition is presented as an explained offer rather than a silent install.

8. **Version, docs, and site.** `plugin.json` → 0.3.0 with an updated description and keywords. `CHANGELOG.md` 0.3.0 entry including a "Notes for existing users" section. `README.md` — the two optional stages added to the stage explanation, day-to-day usage, the plugin tree, the superpowers-relationship list, and Governance; Status section rewritten. **"Research preview" retired everywhere** — the label undersold a plugin running against real production codebases. Replaced with the version and the license, per Decision (Session 7, no number — a wording call, not an architecture one).

9. **`docs/feature-workflow.svg`** — canvas grown 950 → 1290. New dashed VERIFY band with `browser-testing` and the human's own browser pass shown side by side as **concurrent**, `review-loop` beneath them, then a gold human gate for the full test suite before the outcome box. New legend entry and a `verify` gradient. Rendered and visually checked at 2× before committing.

10. **`aiforging.github.io`** — a "Then you check it. OPTIONAL" subsection added to the Method section with two dashed cards and a gold-rule callout for the full-suite handoff; badge, JSON-LD `softwareVersion`, both FAQ copies, and the plugin note updated; install snippet extended with `forge` / `browser-testing` / `review-loop`; `sitemap.xml` lastmod bumped. **The OneDrive copy at `OneDrive-CertainPath/ai-forging/website/` had already drifted three weeks and 10 KB behind the deployed repo before this session** — it was still serving the v0.2.0 page. Resolved at the end of the session: the whole folder was re-synced from `projects/aiforging.github.io`, and a `README.md` was dropped in it naming the repo as the source of truth and saying not to edit the mirror. Two sources for one page is how the next edit gets lost, and this one already had.

11. **Open-source housekeeping.** `CONTRIBUTING.md` (the field-report ask, the opinionated-about list, the pattern-file bar, stack-adapter requirements, the manifest rule, the dogfood loop). `.github/ISSUE_TEMPLATE/` with a field-report form and a pattern-proposal form, plus a config with contact links. `.github/pull_request_template.md` whose checklist encodes the manifest rule and the mirrored-docs rule.

12. **Audit pass, and what it caught.** An independent subagent audited the session's output against the repo before anything was committed. It found nine real defects, all fixed:

    - **The two new stages were documented as slash commands in ~15 places, but only existed as skills.** Skills are not `/`-invocable. Fixed by adding `commands/browser-testing.md` and `commands/review-loop.md` as thin pointers (the `forge.md` → `new-feature.md` pattern), which was also the shape the ask implied. **Related:** `commands/setup.md` Step B.5 had been telling users since v0.1.0 that they could invoke `/aiforging:hammer-refactor` and `/aiforging:capture-pattern` — skills with no command wrapper, so the sentence was simply false, and had been through two releases without anyone noticing. Resolved by adding those two pointers as well, so **every user-invocable skill now has a command**: hammer-refactor, capture-pattern, browser-testing, review-loop. `architecture-analyzer` deliberately has none — it is invoked by `/aiforging:setup` Phase B and is advisory rather than a daily driver; revisit if users start asking for it by name.
    - **A seeded anti-pattern file ordered a full-suite run.** `conventions/refactoring/anti-patterns/primitive-obsession.md` said "Run the full test suite," and it is installed verbatim into every workspace's shared tier and read by exactly the fresh-context Hammer subagent the scoped rule exists to countermand. The rule can be stated in five places and still lose to the one file the agent is actually holding. Scoped it, and the two ambiguous "the test suite" mentions in the other seeded files.
    - **`feature-test-suite.md` contradicted itself** on how many places the rule is repeated — the list predated the two new skills. Now five, with `review-loop`'s fix agents named.
    - **The workspace marker was a fiction.** Three skills checked `docs/features/README.md` for "the AI Forging marker," a string nothing had ever written; `commands/new-feature.md` checked `CLAUDE.md` for `AI Forging forge workspace`, which also appears nowhere, so **that command's in-workspace precondition could never match** — it worked only by falling through to the pointer file. All four now check what setup actually writes (`CLAUDE.md` contains `AI Forging workspace`, plus a `docs/features/` directory). A marker comment was added to `templates/docs-features-README.md` so Decision 21's recorded marker exists, but nothing depends on it.
    - **`review-loop` contradicted itself twice**: it forbade round 3 and permitted it in consecutive bullets (now: hard cap of 2, explicit opt-in per round after), and its stop condition sent the user back to `browser-testing`, which both skills elsewhere declare runs strictly first (now explicit that the ordering rule governs the *first* pass, not the number of observations).
    - **`review-loop` invented a folder shape** — "lettered sub-rounds" — that `conventions/features/README.md` does not define. Replaced with the shapes the convention actually has.
    - **`browser-testing` derived the feature from the branch name**, but nothing in AI Forging creates branches or ties their names to feature folders. Now a hint, never an answer; and it referenced `${CLAUDE_PLUGIN_ROOT}` from a file whose whole purpose when copied is to work without the plugin installed.
    - **The manifest was incomplete on its first pass** — twice. `ANALYSIS.md` was absent although `setup` writes it and `uninstall` removes it, and `uninstall` now derives its list *from* the manifest, so the omission would have left a file behind after a "clean" uninstall. `architecture-analyzer` and `conventions/features/` were absent because they are never copied — a mechanical coverage check caught those two, and a reader would have had to guess whether the omission was deliberate. Both now carry `install: []` with a note. **The manifest is only useful if it is exhaustive, including the entries whose answer is "nowhere."**
    - **Mirror drift already, in the same session that added the mirroring rule.** The `## Test suite` block that both feature docs tell authors to paste verbatim into `plan.md` differed by one sentence — the mirror had dropped the line assigning the full-suite run to the human, so workspace-authored plans would have carried the weaker version. Two hard rules existed only in the mirror and two only in the canonical doc. Canonical gained a proper `## Hard rules` section; both are now aligned.

    **The lesson worth keeping:** every one of these was invisible from inside the work. Four were self-contradictions inside a single file, and the two most dangerous — the seeded file overriding the scoped rule, and the manifest omission that would survive an uninstall — were places where the *authoritative* artifact disagreed with the prose describing it. A mechanical coverage check found what prose review would not, and an adversarial read found what the mechanical check could not. Run both before a release, not one.

13. **The documented upgrade path had never worked — found by trying it.** Testing `/aiforging:update-targets` against ServiceLine (Scenario C, single repo) failed before reaching the command at all: the README's `/plugin update aiforging@aiforging` is not a real command (updating an installed plugin is the shell command `claude plugin update`; there is no slash form), and once corrected it still failed with `Plugin "aiforging" is not installed at scope user`.

    **The scope failure is the interesting one, because it is systemic rather than incidental.** `/aiforging:setup` deliberately writes `enabledPlugins` into the *project's* `.claude/settings.json` so teammates who clone a repo get the plugin activated without touching personal config — a good decision (see "install vs enable" in the README). The unnoticed consequence is that `aiforging` and `superpowers` end up installed at **project** scope, while `claude plugin update` defaults to `--scope user`. So **the documented upgrade path was broken for precisely the users who followed the documented install path**, and the error message reads like "not installed" when it means "wrong scope." Confirmed on ServiceLine: `aiforging@aiforging`, Version 0.2.0, Scope project; `superpowers@claude-plugins-official`, Scope project.

    Upgrading now starts with `claude plugin list` — read your own marketplace and scope off it, then substitute both — and the error is quoted verbatim in the README with an explanation. `CONTRIBUTING.md`'s testing checklist also now says to confirm the version actually moved before running `update-targets`, since propagating from a plugin that did not update produces a "nothing to do" that looks like a bug in the command rather than a skipped upgrade.

    **Worth sitting with:** this was in the README for two releases, in the section a user reaches only when they are already invested enough to upgrade. Nobody hit it because nobody had upgraded — the same reason `update-targets` itself remains unproven. Instructions that have never been executed are not documentation, they are a hypothesis. The v0.3.0 audit checked internal consistency thoroughly and could not have caught this, because the claim was about the outside world.

**Propagation gap (standing).** Existing target repos hold *copies* of `hammer-refactor`, `capture-pattern`, and the conventions. Everything changed in items 2 and 3 above lands in those copies only via `/aiforging:update-targets`. That command is now manifest-driven and should handle it — **but it has still never been dogfooded end to end.** That is the single highest-value thing to do next, because this release is the first one that genuinely depends on it working.

**Not done this session, deliberately:**

- No dogfooding of anything written here. Every skill, command, and convention in this session is written but unrun.
- Neither new skill has been executed against a real feature. `review-loop`'s capability-verification step and its fallback rubric are the least certain parts — the fallback has never been exercised.
- `browser-testing` assumes browser automation is available in the session but deliberately does not name a provider. Whether that abstraction survives contact with a real setup is unknown.
- Decision 20 (upstream pattern promotion) deferred again.

**Next session opening moves:**

1. **Dogfood `/aiforging:update-targets` 0.2.0 → 0.3.0** against a real workspace with at least two onboarded targets, one of them onboarded before the two-tier model. This is the release's load-bearing assumption.
2. **Run `browser-testing` against a real feature with a real `testing.md`.** Watch specifically for: whether the 👤 marking pass is too conservative or not conservative enough, and whether the browser-agnostic preconditions actually let it start.
