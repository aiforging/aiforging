...
Last updated: 2026-04-10 (Session 3 continued — PLAN.md writing bug fix + capture-pattern skill)
...
- `commands/setup.md` — the `/aiforging:setup` slash command. Two phases (A: init-workspace, B: onboard-project) with explicit phase detection, split settings files, git integration subroutine, uv/python3 runner probe, and a Hard Rule forbidding writes under `${CLAUDE_PLUGIN_ROOT}`.
...
- `conventions/` library (copied into each onboarded target as `.aiforging/`):
  ...
  - `conventions/features/README.md` — canonical feature-folder convention including the AI Forging slice format for `plan.md`.
